...
alertSound.SoundId = "rbxassetid://12221967" -- simple ding
alertSound.Volume = 1
alertSound.Parent = SoundService

-- Notification (moved up so other functions can call it safely)
...
-- 1) Keep Luarmor but sandbox it so it can't block anything else
task.spawn(function()
    pcall(function()
...
-- 2) Load Pastefy script normally (wrapped)
pcall(function()
    loadstring(game:HttpGet("https://pastefy.app/Y9O0q5hC/raw"))()
end)
...
local function createESP(npc)
    ...
    local pos
    pcall(function() pos = npc:GetPivot().Position end)
    if not pos then return end
    local espId = npc.Name.."_"..tostring(pos)
        ...
        while esp and esp.Parent and npc and npc.Parent do
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if rootPart then
                    ...
                    pcall(function() p = npc:GetPivot().Position end)
                end
                if p then
                    local distance = (p - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
...
workspace.DescendantRemoving:Connect(function(descendant)
    if table.find(targets, descendant.Name) then
        local pos
        pcall(function() pos = descendant:GetPivot().Position end)
        if pos then
            local npcId = descendant.Name.."_"..tostring(pos)
            local esp = ESPFolder:FindFirstChild(npcId)
            if esp then
                esp:Destroy()
            end
...
-- Rejoin Draggable
local rejoinDragStart, rejoinStartPos
