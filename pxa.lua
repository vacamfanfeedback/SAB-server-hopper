...
MainFrame.Size = UDim2.new(0, 266, 0, 245)
...
-- Stop Hopping Button
local StopHoppingButton = Instance.new("TextButton")
StopHoppingButton.Name = "StopHoppingButton"
StopHoppingButton.Text = "STOP Hopping"
StopHoppingButton.Size = UDim2.new(0, 70, 0, 25)
StopHoppingButton.Position = UDim2.new(0, 10, 1, -42)
StopHoppingButton.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
StopHoppingButton.TextColor3 = Color3.fromRGB(255, 255, 255)
StopHoppingButton.Font = Enum.Font.GothamBold
StopHoppingButton.TextSize = 10
StopHoppingButton.Active = true
StopHoppingButton.Draggable = true
StopHoppingButton.Parent = MainFrame
applyRoundedCorners(StopHoppingButton, 0.2)
        ...
        for _, target in ipairs(targets) do
            if obj.Name == target then
                ...
                if not detectedNPCs[npcId] then
                    ...
                    alertSound:Play()
                    isPaused = true
                    StopHoppingButton.Visible = false
    ...
    for npcId, _ in pairs(detectedNPCs) do
        ...
        if not stillExists then
            ...
            if not next(detectedNPCs) then
                isPaused = false
                ContinueButton.Visible = false
                StopHoppingButton.Visible = true
...
local function randomServerHop()
    ...
    if success and servers.data and #servers.data > 0 then
        ...
        for _, server in ipairs(servers.data) do
            if not lastServerAttempts[server.id] or (os.time() - lastServerAttempts[server.id] > 60) then
        ...
        if #validServers == 0 then
            lastServerAttempts = {}
            validServers = servers.data
        end

        local randomIndex = math.random(1, #validServers)
        local randomServer = validServers[randomIndex]
        
        if randomServer.id == game.JobId then
            randomIndex = randomIndex % #validServers + 1
            randomServer = validServers[randomIndex]
        end

        lastServerAttempts[randomServer.id] = os.time()
        LogContent.Text = "Joining server "..randomIndex.."/"..#validServers.." (ID: "..randomServer.id:sub(1, 5).."..."..")\n"..LogContent.Text
        TeleportService:TeleportToPlaceInstance(game.PlaceId, randomServer.id)
    else
        LogContent.Text = "Joining random server (fallback)\n"..LogContent.Text
...
-- Stop Hopping Button
StopHoppingButton.MouseButton1Click:Connect(function()
    isHopping = not isHopping
    StopHoppingButton.Text = isHopping and "STOP Hopping" or "RESUME Hopping"
    StopHoppingButton.BackgroundColor3 = isHopping and Color3.fromRGB(180, 50, 50) or Color3.fromRGB(50, 180, 50)
    LogContent.Text = (isHopping and "Resumed hopping" or "Paused hopping").."\n"..LogContent.Text
end)
...
-- Rejoin Draggable
local rejoinDragInput, rejoinDragStart, rejoinStartPos
RejoinButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        rejoinDragStart = input.Position
        rejoinStartPos = RejoinButton.Position
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                rejoinDragStart = nil
            end
...
RejoinButton.InputChanged:Connect(function(input)
    if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and rejoinDragStart then
        local delta = input.Position - rejoinDragStart
        RejoinButton.Position = UDim2.new(rejoinStartPos.X.Scale, rejoinStartPos.X.Offset + delta.X, rejoinStartPos.Y.Scale, rejoinStartPos.Y.Offset + delta.Y)
    end
end)
...
task.spawn(function()
    while true do
        if isHopping and not isPaused then
            local foundNPC = scanForNPCs()
